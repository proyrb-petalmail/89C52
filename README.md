...
- **Timer**
  ...
  > 简介：`定时器0`计满时执行的`中断函数`。
  >
  > 注意：该函数无需通过代码调用，而是由单片机自动中断来执行；但库文件中只是声明了该函数，并未定义该函数，所以请在使用前先对该函数进行定义（不同的定时器对应的储存地址不同，即在定义不同定时器的中断函数时指定的数字不同，`定时器0`的中断数字为`1`）；示例如下：
  >
  > ```c
  ...
  > 简介：`定时器1`计满时执行的`中断函数`。
  >
  > 注意：该函数无需通过代码调用，而是由单片机自动中断来执行；但库文件中只是声明了该函数，并未定义该函数，所以请在使用前先对该函数进行定义（不同的定时器对应的储存地址不同，即在定义不同定时器的中断函数时指定的数字不同，`定时器1`的中断数字为`3`）；示例如下：
  >
  > ```c
